...
function _Main$main$AS(args) {
	...
	function getOptimizationLevel() {
		...
		var i;
		var item;
		items = dom.document.getElementsByName("optimization-level");
	...
	function jsxComplete(cm) {
		CodeMirror.showHint(cm, (function (editor) {
			...
			completionRequest = ({_lineNumber: cur.line + 1, _columnOffest: cur.ch, _candidates: []});
			path = "<source>";
			platform = new BrowserPlatform();
			Platform$setFileContent_0$LPlatform$SS(platform, path, editor.getValue());
			c = new Compiler(platform);
			emitter = new JavaScriptEmitter(platform);
			Compiler$setEmitter_0$LCompiler$LEmitter$(c, emitter);
			Compiler$setMode_0$LCompiler$N(c, Compiler.MODE_COMPLETE);
			Compiler$addSourceFile_0$LCompiler$LToken$SLCompletionRequest$(c, null, path, completionRequest);
			Compiler$compile_0$LCompiler$(c);
			ret = ({  });
			uniq = {};
			ret.list = CompletionRequest$getCandidates_0$LCompletionRequest$(completionRequest).map((function (item) {
				return (item.partialWord || item.word) + "";
			})).filter((function (item) {
				if (! $__jsx_ObjectHasOwnProperty.call(uniq, item)) {
					uniq[item] = true;
					return true;
				} else {
					return false;
	...
	list = dom.document.getElementById('source-list');
	input = dom.document.getElementById('input');
	editor = CodeMirror.fromTextArea(input, ({ mode: "application/jsx", lineNumbers: true, lineWrapping: true, autofocus: true, extraKeys: ({ "Ctrl-Space": jsxComplete, "Ctrl-P": jsxComplete, ".": (function (editor) {
		var cursor;
	...
	output = CodeMirror.fromTextArea(dom.document.getElementById("output"), ({ mode: "javascript", lineNumbers: true, lineWrapping: true, readOnly: true }));
	console_log = js.global.console.log;
	function compile(options) {
		...
		platform = new BrowserPlatform();
		content$0 = editor.getValue();
		platform.fileContent[path] = content$0;
		c = new Compiler(platform);
		emitter = new JavaScriptEmitter(platform);
		c._emitter = emitter;
		...
		case "parse":
			...
			break;
		default:
			dom.window.alert("unknown mode: " + (options.mode + ""));
			return;
		}
		if (options.mode == "run") {
			js.global.console.log = (function (arg) {
				output.setValue(output.getValue() + arg + "\n");
			});
		} else {
			js.global.console.log = console_log;
		}
		Compiler$addSourceFile_0$LCompiler$LToken$S(c, null, path);
		success = Compiler$compile_0$LCompiler$(c);
		if (success) {
			if (options.mode == 'parse') {
				output.setOption("mode", "javascript");
				output.setValue(JSON.stringify(Compiler$getAST_0$LCompiler$(c), null, 2));
				return;
			}
			out = emitter.getOutput$().replace(/\t/g, "  ");
			out += "JSX.require('" + path + "')._Main.main$AS([]);\n";
			level = getOptimizationLevel();
			if (level > 1 && options.mode != 'parse') {
				out = BrowserPlatform$applyClosureCompiler_0$LBrowserPlatform$SSB(platform, out, (level <= 2 ? "SIMPLE_OPTIMIZATIONS" : "ADVANCED_OPTIMIZATIONS"), false);
			}
			if (options.mode == 'run') {
			...
			if (platform._errors.length !== 0) {
				output.setOption("mode", "");
				output.setValue("ERROR!\n" + platform._errors.join(""));
			}
		}
	}
	function saveInput(input) {
		var session;
		session = ({ inputPath: input.dataset.path, source: editor.getValue(), cursor: editor.getCursor(), optimizationLevel: getOptimizationLevel() });
	...
	function retrieveInput(input) {
		...
		var i;
		var item;
		serializedSession = dom.window.sessionStorage.getItem("jsx.session");
		if (serializedSession) {
			...
			for (i = 0; i < items.length; ++ i) {
				item = items[i];
				if (item.value === session.optimizationLevel + "") {
					item.checked = true;
				} else {
					item.checked = false;
				}
		...
		a.addEventListener('click', (function (event) {
			...
			xhr.onreadystatechange = (function (e) {
				...
				forEach(list.getElementsByTagName("li"), (function (el) {
					var li;
					li = el;
					if (! li.className.match(/\bsource-file\b/)) {
						return;
					}
	...
	dom.window.addEventListener('keyup', (function (e) {
		...
		if (event.ctrlKey) {
			event.preventDefault();
			event.stopPropagation();
			RUN_KEY = "R".charCodeAt(0);
			COMPILE_KEY = "C".charCodeAt(0);
	...
	dom.document.getElementById('run').addEventListener('click', (function (e) {
		compile(({ mode: 'run' }));
	}));
	dom.document.getElementById('compile').addEventListener('click', (function (e) {
		compile(({ mode: 'compile' }));
	}));
	dom.document.getElementById('ast').addEventListener('click', (function (e) {
		compile(({ mode: 'parse' }));
	}));
	retrieveInput(input);
};
...
function CompletionCandidates$makeClassCandidate$LClassDefinition$(classDef) {
	...
	if (docComment) {
		data.doc = docComment._description;
	}
	return data;
};
...
function $__jsx_onload (event) {
	...
	JSX.runMain("system:src/web/playground.jsx", [])
}
